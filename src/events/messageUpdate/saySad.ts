...
export default async (_: Message | undefined, message: Message, client: Client<true>) => {
    ...
    const newMessageIsSad = messageContainsAnyTarget(message, SAD_TARGETS);
    ...
    if(newMessageIsSad) {
        if(!existingMessage) {
            try {
                const reply = await message.reply(`Don't cry over spilt milk! ${await getGifUrl("spilt milk")}`);
        ...
        if(existingMessage) {
            ...
            try {
                await db.delete(sadMessagesTable).where(eq(sadMessagesTable.messageId, message.id));
            }catch {}
        }
    }
};